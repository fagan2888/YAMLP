...
class PartialCharges():
    """
    This class contains the functions required to generate the following variations of  Coulomb matrices
    (with partial charges) for M configurations of N atoms:

    1. **Partial charge coulomb matrix** where the elements along the diagonal are :math:`q_i^2` and the off diagonal
    elements are :math:`q_i q_j/R_{ij}` where :math:`q_i` and :math:`q_j` are the partial charges
    of atom *i* and *j* and :math:`R_{ij}` is the distance between them.

    2. **Partially randomised Coulomb matrix**: same as number 1 but where the partial randomisation described in the
    Coulomb matrix module is applied.

    3. **Partially randomised Coulomb matrix hybrid 1**: the elements along the diagonal are :math:`0.5 Z_i^2.4`
    The off diagonal elements are :math:`q_i q_j/R_{ij}` where :math:`Z_i` is the nuclear charge of atom *i*.

    4. **Partially randomised Coulomb matrix hybrid 2**: the elements along the diagonal are the calculated PBE energies of
    the free atom. The off diagonal elements are :math:`q_i q_j/R_{ij}`.

    :matrixX: a list of lists of atom labels and coordinates. size (n_samples, n_atoms*4)
    :matrixY: a numpy array of energy values of size (N_samples,)
    :matrixQ: a list of numpy arrays containing the partial charges of each atom. size (n_samples, n_atoms)
    ...
    def __generate_pccm(self):
        """
        This function returns the unrandomised partial charge coulomb matrix where the diagonal elements are
        the :math:`q_i^2`. It also calculates the diagonal elements for both the hybrid partial charge matrix.
    ...
    def get_pccm(self):
        """
        This function returns the unrandomised partial charge coulomb matrix with diagonal elements :math:`q_i^2`,
        generated by __generate_pccm().
    ...
    def generatePCCM(self, numRep=5):
        """
        This function generates the partially randomised partial charge Coulomb matrix. The diagonal are :math:`q_i^2`
        and the off diagonal elements are :math:`q_i q_j/R_{ij}` where :math:`q_i` and :math:`q_j` are the partial
        charges of atom *i* and *j* and :math:`R_{ij}` is the distance between them. The descriptor is randomised in the
        same way as the partially randomised Coulomb matrix (with nuclear charges).
        The array obtained contains only the triangular part of the matrix.

        :numRep: number of randomly sorted matrices to generate per sample data - int

        :return:
        :partQCM: numpy array of size (n_samples*numRep, n_atoms*(n_atoms+1)*0.5),
    ...
    def __randomSort(self, X, y, numRep):
        """
        This function randomly sorts all the rows and columns of the coulomb matrices depending on their column norm. It
        generates a matrix of size (n_samples*numRep, n_atoms^2)

        :numRep: The number of randomly sorted matrices to be generated for each data sample.
        :return:
        :ranSort: numpy array of size (n_samples*numRep, n_atoms*(n_atoms+1)/2),
    ...
    def __partial_randomisation(self, X, y_data, numRep, *args):
        """
        This function generates a coulomb matrix with randomisation but where only the columns of elements that are the
        same are swapped around. Otherwise, the atoms are ordered in increasing nuclear charge.

        :X: the (n_samples, n_atoms^2) coulomb matrix to randomise and trim
        :y_data: the y_data in a (n_samples,) shape
        :numRep: The largest number of swaps to do
    ...
    def hybrid_pccm_1(self, numRep=5):
        """
        This function generates the first hybrid of the partial charge Coulomb matrix. The elements along the diagonal
        are :math:`0.5 Z_i^2.4` while the off diagonal elements are :math:`q_i q_j/R_{ij}` where :math:`Z_i` is the
        nuclear charge of atom *i*.

        :numReps: number of randomisations to do per sample (int)
        :return:
        :trim_rand_pccm: (n_samples*n, n_features) numpy array with partial charge coulomb matrix.
        :Y_big: numpy array of size (n_samples*min(n_perm, numRep)).
        """
        pccm = self.__generate_pccm()
    ...
    def hybrid_pccm_2(self, numRep=5):
        """
        This function generates the first hybrid of the partial charge Coulomb matrix. The elements along the diagonal
        are the PBE energies of the free atom, while the off diagonal elements are :math:`q_i q_j/R_{ij}`.
